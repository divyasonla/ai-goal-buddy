...
serve(async (req) => {
  ...
  try {
    ...
    const { action, email, username, dailyGoal, reflection, wentWell, challenges, left, status, date, rowIndex } = await req.json();
    ...
    if (action === "fetch") {
      ...
      // Filter by email if provided (students see only their own)
      if (email) {
        goals = goals.filter((g: any) => g.email === email);
      }
    ...
    if (action === "add") {
      ...
      await appendSheetData(accessToken, sheetId, "DailyGoals!A:I", [
        [username, email, dailyGoal, reflection || "", wentWell || "", challenges || "", left || "", today, status || "Pending"],
      ]);
    ...
    if (action === "update") {
      const range = `DailyGoals!A${rowIndex}:I${rowIndex}`;
      await updateSheetRow(accessToken, sheetId, range, [
        [username, email, dailyGoal, reflection || "", wentWell || "", challenges || "", left || "", date, status || "Pending"],
      ]);
      return new Response(JSON.stringify({ success: true }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
